...
// 这些 `SamoyedSystem*` 类型是“系统表面专用的中间结果”：
...
struct SamoyedSystemTaskReference: Equatable, Sendable {
    let date: LocalDay
    let blockID: UUID
    let taskID: UUID
    let title: String
...
struct SamoyedSystemBlockReference: Equatable, Sendable {
    let date: LocalDay
    let blockID: UUID
    let title: String
    let layerIndex: Int
...
struct SamoyedSystemNowSnapshot: Equatable, Sendable {
    let date: LocalDay
    let minuteOfDay: Int
    let currentBlock: SamoyedSystemBlockReference?
    let activeBlocks: [SamoyedSystemBlockReference]
    let topTask: SamoyedSystemTaskReference?
    let remainingTaskCount: Int
    let statusMessage: String?
}

struct SamoyedSystemLiveActivitySnapshot: Equatable, Sendable {
    let date: LocalDay
    let minuteOfDay: Int
    let currentBlock: SamoyedSystemBlockReference?
    let displayBlock: SamoyedSystemBlockReference?
    let displayTasks: [SamoyedSystemTaskReference]
    let displayNote: String?
    let displaySourceBlockTitle: String?
    let remainingTaskCount: Int
    let statusMessage: String?

    var displayTask: SamoyedSystemTaskReference? {
        displayTasks.first
    }

    func tapURL(source: SamoyedSystemSource = .liveActivity) -> URL? {
        // Live Activity 整体点击时，默认跳回 Now 页面。
        SamoyedSystemRoute.now(source: source).url
    }

    func deepLinkURL(source: SamoyedSystemSource = .liveActivity) -> URL? {
        // 更细粒度的 deep link 优先级：
        ...
        return SamoyedSystemRoute.now(source: source).url
    }
}

// `SamoyedSystemActionExecutor` 是系统入口层的小门面(facade)。
// AppIntent / 快捷操作 / widget control 不直接碰 document，而是通过它拿结果。
struct SamoyedSystemActionExecutor {
    let repository: SamoyedDocumentRepository

    init(repository: SamoyedDocumentRepository = .appLive) {
        self.repository = repository
    }

    func currentSnapshot(at date: Date = .now) throws -> SamoyedSystemNowSnapshot {
        try repository.systemNowSnapshot(at: date)
    }

    func openURL(for route: SamoyedSystemRoute) -> URL? {
    ...
    @discardableResult
    func completeCurrentTask(at date: Date = .now) throws -> SamoyedSystemTaskReference? {
...
extension SamoyedDocumentRepository {
    ...
    func completeTask(
        ...
        taskID: UUID,
        completedAt: Date = .now,
        in document: inout SamoyedDocument
    ) throws -> Bool {
    ...
    func systemNowSnapshot(at date: Date) throws -> SamoyedSystemNowSnapshot {
        ...
        if try requiresTemplateSelection(in: document, for: localDay, today: localDay) {
            return SamoyedSystemNowSnapshot(
                date: localDay,
    ...
    func systemNowSnapshot(from now: NowScreenModel) -> SamoyedSystemNowSnapshot {
    ...
    func liveActivitySnapshot(at date: Date) throws -> SamoyedSystemLiveActivitySnapshot {
        let localDay = LocalDay(date: date)
        let document = try preparedDocument(at: date)

        if try requiresTemplateSelection(in: document, for: localDay, today: localDay) {
            return SamoyedSystemLiveActivitySnapshot(
                date: localDay,
                minuteOfDay: date.minuteOfDay,
                currentBlock: nil,
                displayBlock: nil,
    ...
    func liveActivitySnapshot(from now: NowScreenModel) -> SamoyedSystemLiveActivitySnapshot {
    ...
    func currentBlockReference(at date: Date) throws -> SamoyedSystemBlockReference? {
        try systemNowSnapshot(at: date).currentBlock
    }

    func topTaskReference(at date: Date) throws -> SamoyedSystemTaskReference? {
    ...
    func completeTopTask(
        at date: Date,
        completedAt: Date = .now
    ) throws -> SamoyedSystemTaskReference? {
        // “完成当前任务”本质上是：
    ...
    func topTaskReference(
        ...
        return SamoyedSystemTaskReference(
            date: date,
            blockID: blockID,
            taskID: task.id,
            title: task.title,
    ...
    func preparedNowScreenModel(at date: Date) throws -> NowScreenModel {
        ...
        let localDay = LocalDay(date: date)
        let document = try preparedDocument(at: date)
        return try SamoyedPresentation.nowScreenModel(
            document: document,
            date: localDay,
            minuteOfDay: date.minuteOfDay
        )
    ...
    func preparedDocument(at date: Date) throws -> SamoyedDocument {
    ...
    private func materializedDocument(
        from document: SamoyedDocument,
        for date: LocalDay,
        generatedAt: Date
    ) throws -> (document: SamoyedDocument, didMaterialize: Bool) {
    ...
    private func blockReferences(
        ...
        now.activeChain.map { item in
            ...
            return SamoyedSystemBlockReference(
                date: now.date,
                blockID: item.id,
                title: item.title,
    ...
    private func liveActivityDisplaySelection(
        from now: NowScreenModel,
        blocksByID: [UUID: SamoyedSystemBlockReference]
    ) -> (
        block: SamoyedSystemBlockReference,
        tasks: [SamoyedSystemTaskReference],
        note: String?
    )? {
