...
final class SamoyedNotificationCoordinator: NSObject {
    static let shared = SamoyedNotificationCoordinator()

    private let center = UNUserNotificationCenter.current()
    private let identifierPrefix = "samoyed.reminder."
    ...
    func sync(with document: SamoyedDocument, referenceDate: Date = .now) {
        // 同步是异步执行的，因为：
        // - 读取待处理通知需要系统回调
        // - 请求权限 / 添加通知也都是异步 API
        Task {
            ...
            let pendingIdentifiers = Set(
                pending
                    .map(\.identifier)
                    // 只清理属于 Samoyed 的通知，不影响用户设备上其他 app 的内容。
    ...
    private var notificationCategory: UNNotificationCategory {
        ...
        let completeAction = UNNotificationAction(
            identifier: SamoyedSharedConfig.notificationActionCompleteTopTask,
            title: "Complete task"
        )
        let snoozeAction = UNNotificationAction(
            identifier: SamoyedSharedConfig.notificationActionSnooze,
            title: "Remind in 10 min"
        )

        return UNNotificationCategory(
            identifier: SamoyedSharedConfig.notificationCategoryIdentifier,
    ...
    private func request(
        ...
        content.categoryIdentifier = SamoyedSharedConfig.notificationCategoryIdentifier
        content.userInfo = [
            // `userInfo` 是通知跳回 app 时的上下文载荷。
            // 它的地位很像桌面开发里“消息参数”或“命令上下文”。
            SamoyedSharedConfig.notificationUserInfoDateKey: date.description,
            SamoyedSharedConfig.notificationUserInfoBlockKey: block.id.uuidString,
            SamoyedSharedConfig.notificationUserInfoTaskKey: block.tasks
                .sorted(by: taskSort)
    ...
    private func routeURL(for response: UNNotificationResponse) -> URL? {
        ...
        let date = (userInfo[SamoyedSharedConfig.notificationUserInfoDateKey] as? String)
            .flatMap(LocalDay.init(isoDateString:))
        let blockID = (userInfo[SamoyedSharedConfig.notificationUserInfoBlockKey] as? String)
            .flatMap(UUID.init(uuidString:))
        let taskID = (userInfo[SamoyedSharedConfig.notificationUserInfoTaskKey] as? String)
            .flatMap(UUID.init(uuidString:))
    ...
    private func completeTopTask(for response: UNNotificationResponse) async {
        ...
        let userInfo = response.notification.request.content.userInfo
        guard
            let dateString = userInfo[SamoyedSharedConfig.notificationUserInfoDateKey] as? String,
            let date = LocalDay(isoDateString: dateString),
            let blockIDString = userInfo[SamoyedSharedConfig.notificationUserInfoBlockKey] as? String,
        ...
        if #available(iOS 16.1, *) {
            _ = try? await SamoyedCurrentBlockLiveActivityController.sync(using: .appLive, at: .now)
        }
...
extension SamoyedNotificationCoordinator: UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(
        ...
        case SamoyedSharedConfig.notificationActionCompleteTopTask:
            await completeTopTask(for: response)

        case SamoyedSharedConfig.notificationActionSnooze:
        ...
        case UNNotificationDefaultActionIdentifier:
            if let url = routeURL(for: response) {
                await MainActor.run {
                    ...
                    SamoyedExternalRouteCenter.shared.enqueue(url)
                }
            }
