...
private enum SamoyedIntentError: LocalizedError {
    case missingRoute
...
struct OpenNowIntent: AppIntent {
    ...
    static let description = IntentDescription("Open Samoyed to the Now screen.")
    static let openAppWhenRun = false

    func perform() async throws -> some IntentResult & OpensIntent {
        // `OpensIntent` 表示这个 intent 的结果是“让系统继续执行一个打开动作”。
        guard let url = SamoyedSystemRoute.now(source: .shortcut).url else {
            throw SamoyedIntentError.missingRoute
        }
...
struct OpenTodayIntent: AppIntent {
    ...
    static let description = IntentDescription("Open Samoyed to today's timeline.")
    static let openAppWhenRun = false

    func perform() async throws -> some IntentResult & OpensIntent {
        guard let url = SamoyedSystemRoute.today(
            date: .today(),
            blockID: nil,
            taskID: nil,
            source: .shortcut
...
struct OpenCurrentBlockIntent: AppIntent {
    ...
    static let description = IntentDescription("Open Samoyed to the current active block.")
    static let openAppWhenRun = false

    func perform() async throws -> some IntentResult & OpensIntent & ProvidesDialog {
        // `SamoyedSystemActionExecutor` 是“系统表面上的动作执行器”：
        ...
        let url = try executor.openCurrentBlockURL(at: .now, source: .shortcut)
            ?? SamoyedSystemRoute.now(source: .shortcut).url
...
struct EndCurrentBlockLiveActivityIntent: AppIntent {
    ...
    static let description = IntentDescription("End the current Samoyed Live Activity.")
    static let openAppWhenRun = false
...
struct SamoyedShortcutsProvider: AppShortcutsProvider {
